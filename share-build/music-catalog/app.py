...
_sync_state = {'running': False, 'added': 0, 'skipped': 0, 'errors': 0, 'done': 0, 'total': 0, 'finished': False, 'error': None}

def _run_discogs_sync(token, username, records):
...
def settings_sync_discogs():
    ...
    conn = database.get_db()
    records = conn.execute(
        "SELECT id, discogs_url FROM collection "
    ...
    t.start()
    return jsonify({'ok': True, 'started': True, 'total': len(records)})
